...
configure_aws_cli() {
    ...
    if ! aws sts get-caller-identity >/dev/null 2>&1; then
        ...
        echo "AWS CLI configured with default region 'eu-west-1' and output format 'json'."
    else
        ...
        if [[ "$choice" == [Yy]* ]]; then
            ...
            echo "AWS CLI reconfigured with default region 'eu-west-1' and output format 'json'."
        else
...
run_scripts() {
    # Run generate_tfvars.sh with the required arguments
    echo "Running generate_tfvars.sh..."
    bash "$GENERATE_TFVARS_SCRIPT" "$1" "$2"
    if [ $? -ne 0 ]; then
...
main() {
    ...
    if [ "$#" -ne 2 ]; then
        echo "Usage: $0 <owner> <environment>"
        exit 1
    fi
